...
export default function PublicationsAdminPage() {
  ...
  useEffect(() => {
    if (firestore) fetchPublications();
  }, [firestore]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingPub) {
        await updateDoc(doc(firestore, "publications", editingPub.id), pubData);
        toast({ title: "Succès", description: "Publication mise à jour" });
      } else {
        await addDoc(collection(firestore, "publications"), {
          ...pubData,
            ...
            <form onSubmit={handleSubmit} className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={category} onValueChange={setCategory}>
                  ...
                  <SelectContent>
                    <SelectItem value="Revue REEM">Revue REEM</SelectItem>
                    <SelectItem value="Ouvrages Spéciaux">Ouvrages Spéciaux</SelectItem>
                    <SelectItem value="Rapports de Recherche">Rapports de Recherche</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
